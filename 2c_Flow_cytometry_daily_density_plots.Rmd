...
flow_model_estimate = readRDS(paste0(filepath, "/results/2022-12-31_density_estimate_plot.RDS"))
...
ggplot2::ggsave(filename = paste0(filepath_figures, "figure3_density_plot.svg"), width = 180, height = 70, units = "mm")
```
...
bacterial_density  %>% 
  filter(PP == "H") %>% 
  group_by(Group, DPI) %>% 
  summarise(mean = mean(density_cells_mL)) %>% View()
```
