...
lm_df = bacterial_density %>% 
  dplyr::select(density_cells_mL, DPI, Added_Fc7, Treatment) %>%
...
# untransformed
```{r}
linear = lm(formula = density_cells_mL ~ DPI * Treatment * Added_Fc7,  data = lm_df)
poly2 = lm(formula = density_cells_mL ~ Treatment * poly(DPI, degree = 2)*Added_Fc7,  data = lm_df)
poly3 = lm(formula = density_cells_mL ~ Treatment * poly(DPI, degree = 3) * Added_Fc7,  data = lm_df)
poly4 = lm(formula = density_cells_mL ~ Treatment * poly(DPI, degree = 4) * Added_Fc7,  data = lm_df)

anova(linear, poly2, poly3, poly4)
AIC = AIC(linear, poly2, poly3, poly4)$AIC
r2 = c(summary(linear)$adj.r.squared, summary(poly2)$adj.r.squared, summary(poly3)$adj.r.squared, summary(poly4)$adj.r.squared) 
#r sqr val
...
```{r}
linear = lm(formula = log10(density_cells_mL) ~ DPI * Treatment * Added_Fc7,  data = lm_df)
poly2 = lm(formula =  log10(density_cells_mL) ~ Treatment * poly(DPI, degree = 2)*Added_Fc7,  data = lm_df)
poly3 = lm(formula =  log10(density_cells_mL) ~ Treatment * poly(DPI, degree = 3) * Added_Fc7,  data = lm_df)
poly4 = lm(formula =  log10(density_cells_mL) ~ Treatment * poly(DPI, degree = 4) * Added_Fc7,  data = lm_df)
...
## rank
```{r}
lm_df_no_P = bacterial_density %>% mutate(microcosm = substr(Sample, 1,5)) %>%
  dplyr::select(density_cells_mL, DPI, Added_Fc7, Treatment, microcosm) %>%
  dplyr::mutate(microcosm = factor(microcosm), log_density = log(base = 10, x = density_cells_mL))  

linear = lm(formula = rank(density_cells_mL) ~ DPI * Treatment * Added_Fc7,  data = lm_df_no_P)
poly2 = lm(formula =  rank(density_cells_mL) ~ Treatment * poly(DPI, degree = 2)*Added_Fc7,  data = lm_df_no_P)
poly3 = lm(formula =  rank(density_cells_mL) ~ Treatment * poly(DPI, degree = 3) * Added_Fc7,  data = lm_df_no_P)
poly4 = lm(formula =  rank(density_cells_mL) ~ Treatment * poly(DPI, degree = 4) * Added_Fc7,  data = lm_df_no_P)
...
set.seed(3003)
poly3 = lm(formula =  log10(density_cells_mL) ~ Treatment * poly(DPI, degree = 3) * Added_Fc7,  data = lm_df)
# step(poly3) step did not indicate that any coefficients should be removed
summary(poly3)
anova(poly3)
...
report = data.frame(Coefficient = rownames(lm.summary$coefficients), 
           ...
           p = format( round(lm.summary$coefficients[,4],3), nsmall = 1 )
           ) 
...
estimates_day =EMM$contrasts %>%#get confidence intervals and p value per day
     summary(infer = TRUE) %>% 
  arrange(contrast) %>% 
  data.frame()
...
saveRDS(object = df, file = paste0(filepath_results, "density_estimate_plot.RDS"))
```
