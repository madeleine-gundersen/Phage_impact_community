...
## lmer
the variance explained by the microcosm comparison has lower variance than the residuals. Therefore switchin to a unmixed linear model
```{r}
# rep_sim_DNA
# linear = lmerTest::lmer(formula = BrayCurtis_sim ~ DPI * Treatment * Added_Fc7 +
#                           (1 + DPI| Rep_comp:Treatment), #random effect; each microcosm hasa different intercept and varying slopes
...
## lm
the variance explained by the microcosm comparison has lower variance than the residuals. Therefore switchin to a linear model
```{r}
set.seed(3003)
linear_DNA_bray = lm(formula = BrayCurtis_sim ~ DPI * Treatment * Added_Fc7, 
            data = rep_sim_DNA.2_bray)
...
coeffs = coefficients(model_DNA_bray) %>% data.frame() %>% rename(Estimate = ".")
...
```{r}
set.seed(3003)
linear_DNA_sor = lm(formula = Sorensen_sim ~ DPI * Treatment * Added_Fc7, 
...
coeffs = coefficients(model_DNA_sor) %>% data.frame() %>% rename(Estimate = ".")
coeffs$Coefficients = rownames(coeffs)
...
ggsave(filename = paste0(filepath_figures, "Figure7_Replicate-similarity-rate.pdf"), width = 180, height = 60, units = "mm")
...
rates %>% 
  ...
  dplyr::group_by(Added_Fc7) %>% 
  dplyr::summarise(mean_p = round(mean(phage_diff),4), 
                   mean_ab = round(mean(ab_diff),4))

rates %>% select(distance, Extraction, Added_Fc7, Treatment, rate) %>% 
  tidyr::spread(value = rate, key = Treatment) %>% 
  dplyr::mutate(phage_diff = none-Phage, ab_diff = none - AB) %>% 
  dplyr::group_by(Added_Fc7) %>% filter(Added_Fc7 == "no")
  dplyr::summarise(mean_p = round(mean(phage_diff),4), 
                   mean_ab = round(mean(ab_diff),4))
  
rates %>% select(distance, Extraction, Added_Fc7, Treatment, rate) %>% 
  tidyr::spread(value = rate, key = Treatment) %>% 
  dplyr::group_by(distance, Extraction, Added_Fc7,  ) %>%
